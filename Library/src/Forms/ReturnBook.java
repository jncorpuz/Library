...
public class ReturnBook extends JFrame {
    ...
    private void initComponents()
    {
        ...
        cmdReturn.setFont(new java.awt.Font("Tahoma", 1, 12)); // NOI18N
        ...
        getContentPane().add(cmdReturn, new org.netbeans.lib.awtextra.AbsoluteConstraints(190, 330, 110, 40));
    ...
    private void cmdReturnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdReturnActionPerformed
        ...
        try{
            ...
            if(result.isBeforeFirst())
            {
                ...
                {
                    statement = "update borrowinfo set returnDate=CURRENT_DATE where id='" + borrowID +"';";
                    dbCom.execute(statement);
                }
                else
                {
                    statement = "SELECT penaltyrecords.id from penaltyrecords, borrowinfo WHERE penaltyrecords.borrowInfoID=borrowinfo.id and borrowinfo.id='" + borrowID + "';";
                    result = dbCom.executeQuery(statement);
                    
                    if (!result.isBeforeFirst())
                    {
                        JOptionPane.showMessageDialog(null, "Please Settle Penalty First at Penalty Management");
                    }
                }
            }
        }
